...
describe("JwtVerifier", () => {
  it("verifica un token HS256 y devuelve su identidad", async () => {
  ...
  it("rechaza un token firmado con otro secreto", async () => {
    const token = jwt.sign({ sub: "u1" }, "x".repeat(32), { algorithm: "HS256" });
  ...
  it("rechaza un token sin claim sub", async () => {
    const token = jwt.sign({ role: "player" }, SECRET, { algorithm: "HS256" });
